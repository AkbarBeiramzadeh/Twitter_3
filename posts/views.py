...
class PostsView(View):
    ...
    def get(self, request):
        ...
        if request.GET.get('search'):
            posts = posts.filter(text__contains=request.GET['search'])
        return render(request, 'posts/posts.html', {'posts': posts, 'form': self.form_class})
...
class PostUpdateView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            new_post.slug = slugify(form.cleaned_data['text'][:30])
            new_post.save()
